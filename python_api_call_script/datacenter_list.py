...
api_url = 'https://bcb-hci-vc-001.ahs.cos.lan/rest'
api_user = 'adminir@vsphere.local'
api_pass = ''

def main():
    requests.packages.urllib3.disable_warnings(InsecureRequestWarning) #Disable SSL warnings
    get_vcenter_datacenter_status()

def get_vcenter_datacenter_status():
    resp = get_api_data('{}/vcenter/datacenter'.format(api_url))
    j = resp.content
    #j = resp.json()
    print('DataCenter:  {}'.format(j))
